import { MoreHorizontal, Edit2, Trash2, LayoutGrid, List, Moon, Sun, Download, Upload, FileSpreadsheet, MessageCircle, RefreshCw, Trash } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import React, { useState, useRef, useEffect } from 'react';
import { MainIcon, WriteOffIcon, FinanceIcon, ShiftIcon } from './CustomIcons';
...
export default function Dashboard({ partsCount, operationsCount, onOpenParts, onOpenOperations, onViewFinance, onViewShifts, isDark, onThemeToggle, onExport, onImport, onExportExcel, onImportExcel, onCsv, onTelegramSettings, onUpdateApp, onClearData }: DashboardProps) {
  ...
  const cards = [
    {
      key: 'parts' as const,
      icon: <div className="w-[72px] h-[72px] rounded-2xl bg-blue-600/10 flex items-center justify-center shadow-[inset_0_0_20px_rgba(59,130,246,0.1)] transition-all group-hover:scale-105"><MainIcon className="w-12 h-12 text-blue-400" /></div>,
      title: 'Основной',
    ...
    },
    {
      ...
      icon: <div className="w-[72px] h-[72px] rounded-2xl bg-blue-500/10 flex items-center justify-center shadow-[inset_0_0_20px_rgba(59,130,246,0.1)] transition-all group-hover:scale-105"><WriteOffIcon className="w-12 h-12 text-blue-400" /></div>,
      title: 'Журнал\nсписаний',
    ...
    {
      key: 'finance',
      icon: <div className="w-[72px] h-[72px] rounded-2xl bg-blue-400/10 flex items-center justify-center shadow-[inset_0_0_20px_rgba(59,130,246,0.1)] transition-all group-hover:scale-105"><FinanceIcon className="w-12 h-12 text-blue-400" /></div>,
      title: 'Финансовый\nжурнал',
    ...
    {
      key: 'shifts',
      icon: <div className="w-[72px] h-[72px] rounded-2xl bg-blue-500/10 flex items-center justify-center shadow-[inset_0_0_20px_rgba(59,130,246,0.1)] transition-all group-hover:scale-105"><ShiftIcon className="w-12 h-12 text-blue-400" /></div>,
      title: 'Журнал\nсмен',
      sub: 'Мои смены',
          ...
          <div className="grid grid-cols-2 gap-3">
            {cards.map(card => (
              <motion.div
                ...
                className={`group rounded-[24px] p-5 flex flex-col cursor-pointer relative aspect-square ${isDark ? 'bg-[#242426] shadow-[0_10px_30px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.05)] border border-white/[0.02]' : 'bg-white shadow-md border border-gray-100'}`}
              >
          ...
          <div className="flex flex-col gap-3">
            {cards.map(card => (
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                onClick={card.onClick}
                className={`group rounded-[20px] px-5 py-4 flex items-center gap-4 cursor-pointer relative ${isDark ? 'bg-[#242426] shadow-[0_4px_16px_rgba(0,0,0,0.5)] border border-white/[0.02]' : 'bg-white shadow-md border border-gray-100'}`}
              >
